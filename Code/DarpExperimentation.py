    ...
    while currentTime <= timeLimit: # the algorithm ends when the time limit is reached
        ...
        if len(availableRequests) > 0: # if there are requests we can serve, we should serve them
            ...
            if len(availableRequests) > 0 and not currentRequest[1] == availableRequests[0][0]: # check if a jump needs to be made
                #print(str(currentRequest[1]) + "->" + str(availableRequests[0][0]))
                currentTime+=1 # a jump will be required, so we must increment the current time by one time unit to reflect this
                #availableRequests = updateRequests(currentTime+1, requests, windowSize) # serving a request means we must update the available requests, this is due to the current time changing (previously existing requests may be unservable)
                timeServed.append('x')
...
def runTestCases(testFolder):
    ...
    """
    #for file in glob.glob(testFolder + "\\test*Paper.txt"): # find every file in the specified folder that is a test file (test#.txt)
