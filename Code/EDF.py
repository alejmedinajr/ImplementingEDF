...
def updateRequests(currentTime, requests, windowSize, timelimit):
    """
...
def edf(graph, timeLimit):
    ...
    while currentTime < timeLimit and len(requests) + len(availableRequests) > 0 and windowSize < timeLimit: # the algorithm ends when the time limit is reached
        ...
        availableRequests = updateRequests(currentTime, requests, windowSize, timeLimit) # the available requests needs to be updated at the start of every iteration before we serve any rides
        ...
        if len(availableRequests) > 0: # if there are requests we can serve, we should serve them
            ...
            availableRequests = updateRequests(currentTime, requests, windowSize, timeLimit) # serving a request means we must update the available requests, this is due to the current time changing (previously existing requests may be unservable)
    ...
    timeTaken = end - start    
    return ridesServed, timeServed, requestsServed, timeTaken # return the number of requests that were served by the EDF algorithm